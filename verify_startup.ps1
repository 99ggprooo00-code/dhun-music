# verify_startup.ps1
# Builds Dhun, launches it, waits 60s, kills it, then parses the startup log.
# Outputs: STARTUP_TOTAL_MS=<number>
# Exit code: 0 on success, 1 on failure.

$ErrorActionPreference = 'Stop'

$startupLogPath = "$env:USERPROFILE\Downloads\dhun_startup.log"

# ── 1. Build ──────────────────────────────────────────────────────────────────
Write-Host "[verify] Building Dhun.WinUI..." -ForegroundColor Cyan
...
& $msbuild src\Dhun.WinUI\Dhun.WinUI.csproj /p:Configuration=Debug /p:Platform=x64 /v:m /nologo
...
$buildOut = "src\Dhun.WinUI\bin\x64\Debug\net10.0-windows10.0.26100\win-x64"
...
# ── 3. Launch ─────────────────────────────────────────────────────────────────
Write-Host "[verify] Launching Dhun..." -ForegroundColor Cyan
explorer.exe shell:AppsFolder\48743Dhun.Dhun_ejgr3wwtkesbm!App
...
# ── 4. Kill the process ───────────────────────────────────────────────────────
Write-Host "[verify] Killing Dhun process..." -ForegroundColor Cyan
$processes = Get-Process -Name "Dhun" -ErrorAction SilentlyContinue
if ($processes) {
    $processes | Stop-Process -Force
    Write-Host "[verify] Process killed." -ForegroundColor Green
} else {
    Write-Warning "[verify] No Dhun process found to kill (may have already exited)."
}
