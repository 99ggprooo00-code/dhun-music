...
public class BackupRestoreService : IBackupRestoreService
{
    ...
    public async Task<BackupResult> CreateBackupAsync(string destinationFolderPath)
    {
        try
        {
            ...
            var stagingPath = Path.Combine(Path.GetTempPath(), $"DhunBackupStaging_{Guid.NewGuid()}");
            ...
            try
            {
                ...
                try
                {
                    var dbPath = Path.Combine(sourcePath, "dhun.db");
                ...
                var zipFileName = $"Dhun_Backup_{timestamp}.zip";
    ...
    public Task<bool> ValidateBackupAsync(string backupFilePath)
    {
        return Task.Run(() =>
        {
            try
            {
                ...
                var hasDb = archive.Entries.Any(e => e.FullName.Equals("dhun.db", StringComparison.OrdinalIgnoreCase));
