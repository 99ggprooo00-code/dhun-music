...
public class ApiKeyService : IApiKeyService, IDisposable
{
    ...
    {
        var serverUrl = _configuration["DhunApiServer:Url"];
        var serverKey = _configuration["DhunApiServer:ApiKey"];
        var subscriptionKey = _configuration["DhunApiServer:SubscriptionKey"];

        if (string.IsNullOrEmpty(serverUrl) || string.IsNullOrEmpty(serverKey))
        {
            _logger.LogCritical("Dhun API Server URL or ApiKey is not configured. API key retrieval will fail.");
            return null;
        }
        ...
        return await _pipelines.ExecuteWithFallbackAsync<string?>(
            ...
            async (response, ct) =>
            {
                ...
                if (response.StatusCode is System.Net.HttpStatusCode.Unauthorized or System.Net.HttpStatusCode.Forbidden)
                {
                    ...
                    _logger.LogCritical(
                        "Authentication failed for Dhun API Server. All future requests disabled. Status: {StatusCode}. Response: {ErrorContent}",
