...
namespace Dhun.WinUI.Services.Implementations;
...
public sealed class LibVlcAudioPlayerService : IAudioPlayer, IDisposable
{
    ...
    // Dhun intentionally keeps approximately 2 dB of headroom below LibVLC's
    ...
    private void InitializeLibVlcCore()
    {
        ...
        _libVlc.SetUserAgent("Dhun", httpUserAgent);
    ...
    private string GetHttpUserAgent()
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(version)) return $"Dhun/{version.Trim()}";
        }
        catch (Exception ex)
        {
    ...
    public void Dispose()
    {
        ...
        try
        {
            ...
            // Release Dhun's media reference. MediaPlayer retains its own reference until
    ...
    private void OnMediaPlayerVolumeChanged(object? sender, MediaPlayerVolumeChangedEventArgs e)
    {
        ...
        // therefore arrives here just like a change made through Dhun. Keep our user-volume
