...
public partial class FolderSongListViewModel : SongListViewModelBase
{
    ...
    public async Task InitializeAsync(string title, Guid? rootFolderId, string? directoryPath = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            TotalItemsText = Dhun.WinUI.Resources.Strings.Folders_ErrorLoading;
    ...
    public override async Task RefreshOrSortSongsAsync(string? sortOrderString = null, CancellationToken manualToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            _dispatcherService.TryEnqueue(() => TotalItemsText = Dhun.WinUI.Resources.Strings.Folders_ErrorLoading);
        }
        finally
        {
    ...
    private void UpdateTotalItemsText(int songCount)
    {
        if (_totalFolderCount > 0 && songCount > 0)
        {
            var folderText = _totalFolderCount == 1
                ? ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Folders_Count_Singular, _totalFolderCount)
                : ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Folders_Count_Plural, _totalFolderCount);
            var songText = songCount == 1
                ? ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Songs_Count_Singular, songCount)
                : ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Songs_Count_Plural, songCount);
        ...
        {
            TotalItemsText = Dhun.WinUI.Resources.Strings.Generic_NoItems;
        }
    }
