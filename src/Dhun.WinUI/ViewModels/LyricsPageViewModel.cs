...
public partial class LyricsPageViewModel : ObservableObject, IDisposable
{
    ...
    [ObservableProperty] public partial string SongTitle { get; set; } = Dhun.WinUI.Resources.Strings.Lyrics_NoSongSelected;
    ...
    private async Task UpdateForTrack(Song? song)
    {
        ...
        _dispatcherService.TryEnqueue(() =>
        {
            ...
            if (song is null)
            {
                ...
                SongTitle = Dhun.WinUI.Resources.Strings.Lyrics_NoSongSelected;
                IsLoading = false;
            }
            else
            {
                SongTitle = !string.IsNullOrWhiteSpace(song.ArtistName)
                    ? string.Format(Dhun.WinUI.Resources.Strings.Lyrics_SongTitleFormat, song.Title, song.ArtistName)
                    : song.Title;
