...
public partial class FolderViewModelItem : ObservableObject
{
    ...
    public void UpdateSongCount(int newSongCount)
    {
        ...
        SongCountText = newSongCount == 1
            ? ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Songs_Count_Singular, newSongCount)
            : ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.Songs_Count_Plural, newSongCount);
    }
}
...
public partial class FolderViewModel : ObservableObject
{
    ...
    private async Task PlayFolderAsync(Guid folderId)
    {
        ...
        {
            _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_Error_Playback;
    ...
    private async Task PlayRandomFolderAsync()
    {
        ...
        try
        {
            ...
            {
                _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_Empty_NoMusicFound;
            }
        }
        catch (Exception ex)
        {
            _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_Error_RandomPlayback;
    ...
    private async Task AddFolderAndScanAsync(string folderPath)
    {
        ...
        var canonicalNew = Dhun.Core.Helpers.PathCanonicalizer.Normalize(folderPath);
        if (!string.IsNullOrEmpty(canonicalNew) &&
            Folders.Any(f => Dhun.Core.Helpers.PathCanonicalizer.Normalize(f.Path)
                .Equals(canonicalNew, StringComparison.OrdinalIgnoreCase)))
        {
            _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_AddFolder_Exists;
            return;
        }

        IsAddingFolder = true;
        _playerViewModel.IsGlobalOperationInProgress = true;
        _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_AddFolder_InProgress;
        ...
        try
        {
            ...
            if (folder == null)
            {
                _playerViewModel.GlobalOperationStatusMessage = Dhun.WinUI.Resources.Strings.Folders_AddFolder_Failed;
                return;
            }
            ...
            _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_AddFolder_Success, folder.Name);
        }
        catch (Exception ex)
        {
            _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Error_AddFolder, ex.Message);
    ...
    private async Task DeleteFolderAsync(Guid folderId)
    {
        ...
        _playerViewModel.IsGlobalOperationInProgress = true;
        _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Delete_InProgress, folderToDelete.Name);
        _playerViewModel.IsGlobalOperationIndeterminate = true;
        ...
        try
        {
            ...
            if (success)
            {
                _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Delete_Success, folderToDelete.Name);
            }
            else
            {
                _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Delete_Failed, folderToDelete.Name);
            }
        }
        catch (Exception ex)
        {
            _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Error_DeleteFolder, ex.Message);
    ...
    private async Task RescanFolderAsync(Guid folderId)
    {
        ...
        IsScanning = true;
        _playerViewModel.IsGlobalOperationInProgress = true;
        _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Rescan_InProgress, folderItem.Name);
        _playerViewModel.IsGlobalOperationIndeterminate = true;
        ...
        try
        {
            ...
            _playerViewModel.GlobalOperationStatusMessage = changesDetected
                ? string.Format(Dhun.WinUI.Resources.Strings.Folders_Rescan_Complete, folderItem.Name)
                : string.Format(Dhun.WinUI.Resources.Strings.Folders_Rescan_NoChanges, folderItem.Name);
        }
        catch (Exception ex)
        {
            _playerViewModel.GlobalOperationStatusMessage = string.Format(Dhun.WinUI.Resources.Strings.Folders_Error_RescanFolder, ex.Message);
