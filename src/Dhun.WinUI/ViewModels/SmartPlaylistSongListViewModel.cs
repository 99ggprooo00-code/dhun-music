...
public partial class SmartPlaylistSongListViewModel : SongListViewModelBase
{
    ...
    public async Task InitializeAsync(string title, Guid? smartPlaylistId, string? coverImageUri = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            TotalItemsText = Dhun.WinUI.Resources.Strings.SmartPlaylist_ErrorLoading;
            Songs.Clear();
        }
    }
    ...
    private static string BuildRuleSummary(SmartPlaylist smartPlaylist)
    {
        ...
        if (ruleCount == 0)
            return Dhun.WinUI.Resources.Strings.SmartPlaylist_RuleSummary_NoRules;

        var matchType = smartPlaylist.MatchAllRules
            ? Dhun.WinUI.Resources.Strings.SmartPlaylist_MatchAll
            : Dhun.WinUI.Resources.Strings.SmartPlaylist_MatchAny;
        var ruleWord = ruleCount == 1
            ? Dhun.WinUI.Resources.Strings.SmartPlaylist_Rule_Singular
            : Dhun.WinUI.Resources.Strings.SmartPlaylist_Rule_Plural;

        return ResourceFormatter.Format(Dhun.WinUI.Resources.Strings.SmartPlaylist_RuleSummary_Format, matchType, ruleCount, ruleWord);
    }
