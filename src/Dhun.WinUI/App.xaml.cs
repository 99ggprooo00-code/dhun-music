...
using Dhun.Core.Data;
using Dhun.Core.Helpers;
using Dhun.Core.Http.Pipelines;
...
using Dhun.WinUI.Services.Implementations;
...
public partial class App : Application
{
    ...
    /// <summary>
    ///     Gets Dhun's default brand accent color, derived from the app logo.
    /// </summary>
    ...
    private static void ConfigureCoreLogicServices(IServiceCollection services)
    {
        ...
        services.AddProviderPipelines(builder =>
        {
            ...
            // Dhun's own API server (key bootstrap). Internal infra; conservative retry.
            builder.AddProvider(new ProviderPolicy
            {
                ProviderId = ServiceProviderIds.DhunApi,
                Channel = new ChannelPolicy
                {
                    PermitsPerWindow = 5,
                    Window = TimeSpan.FromSeconds(1),
    ...
    /// </summary>
    private static async Task BackfillSortKeysAsync(Dhun.Core.Data.MusicDbContext dbContext)
    {
        ...
        if (string.IsNullOrWhiteSpace(logContent))
            ...
            catch (Exception fileEx)
            {
                logContent = string.Format(
                    Dhun.WinUI.Resources.Strings.App_CrashReport_LogFallbackError_Format,
                    originalLogPath,
                    fileEx.Message);
            }
    ...
    private void ShowElevationWarningIfNeededAsync()
    {
        ...
        MainDispatcherQueue?.TryEnqueue(DispatcherQueuePriority.Low, async () =>
        {
            ...
            {
                Title = Dhun.WinUI.Resources.Strings.ElevationWarning_Title,
                Content = Dhun.WinUI.Resources.Strings.ElevationWarning_Message,
                PrimaryButtonText = Dhun.WinUI.Resources.Strings.ElevationWarning_Restart,
                CloseButtonText = Dhun.WinUI.Resources.Strings.ElevationWarning_Continue,
