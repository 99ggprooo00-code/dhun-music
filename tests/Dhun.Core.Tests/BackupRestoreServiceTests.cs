...
public class BackupRestoreServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task ValidateBackupAsync_WithZipContainingDhunDb_ReturnsTrue()
    {
        var zipPath = CreateTempZip(("dhun.db", "database content"));

        var result = await _service.ValidateBackupAsync(zipPath);

        result.Should().BeTrue("a ZIP containing dhun.db is a valid Dhun backup");
    }
    ...
    public async Task ValidateBackupAsync_WithZipMissingBothDbAndSettings_ReturnsFalse()
    {
        ...
        result.Should().BeFalse("a backup missing both dhun.db and settings.json should be rejected");
    }
    ...
    public async Task CreateBackupAsync_CreatesZipContainingSourceFiles()
    {
        ...
        var destDir = CreateTempDir();
        await File.WriteAllTextAsync(Path.Combine(sourceDir, "dhun.db"), "db data");
        ...
        archive.Entries.Should().Contain(e => e.FullName == "dhun.db");
    ...
    public async Task CreateBackupAsync_ExcludesTemporaryAndLockFiles()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(sourceDir, "dhun.db-wal"), "wal");
        await File.WriteAllTextAsync(Path.Combine(sourceDir, "dhun.db-shm"), "shm");
        _pathConfig.AppDataRoot.Returns(sourceDir);
    ...
    public async Task RestoreFromBackupAsync_WithSubdirectoryInBackup_RestoresDirectory()
    {
        ...
        {
            var dbEntry = zip.CreateEntry("dhun.db");
    ...
    {
        // A ZIP without dhun.db or settings.json is not a valid backup.
    ...
    public async Task RestoreFromBackupAsync_WithValidBackup_RestoresFilesToAppDataRoot()
    {
        ...
        result.Success.Should().BeTrue();
        File.Exists(Path.Combine(destDir, "dhun.db")).Should().BeTrue("dhun.db should be restored to AppDataRoot");
        File.Exists(Path.Combine(destDir, "settings.json")).Should().BeTrue("settings.json should be restored to AppDataRoot");
        var restoredDbContent = await File.ReadAllTextAsync(Path.Combine(destDir, "dhun.db"));
